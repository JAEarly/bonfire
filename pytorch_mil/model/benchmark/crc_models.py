...
from pytorch_mil.data.crc.crc_dataset import CRC_N_EXPECTED_DIMS, CRC_FV_SIZE
...
class CrcAttentionNN(models.AttentionNN):
    ...
    def __init__(self, device, n_classes, d_enc=256, ds_enc_hid=(64, 64), ds_agg_hid=(), dropout=0.2, d_attn=128):
        encoder = CrcEncoder(ds_enc_hid, d_enc, dropout)
        aggregator = agg.MultiHeadAttentionAggregator(1, d_enc, ds_agg_hid, d_attn, n_classes, dropout)
        super().__init__(device, n_classes, CRC_N_EXPECTED_DIMS, encoder, aggregator)
...
class CrcMiLSTM(models.AttentionNN):
    ...
    def __init__(self, device, n_classes, d_enc=256, ds_enc_hid=(64, 64), d_lstm_hid=256, n_lstm_layers=1,
