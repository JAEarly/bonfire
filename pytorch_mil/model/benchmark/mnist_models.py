...
from pytorch_mil.data.mnist_bags import MNIST_N_CLASSES, MNIST_N_EXPECTED_DIMS, MNIST_FV_SIZE
...
class MnistInstanceSpaceNN(models.InstanceSpaceNN):
...
class MnistEmbeddingSpaceNN(models.EmbeddingSpaceNN):
...
class MnistAttentionNN(models.AttentionNN):

    def __init__(self, device, d_enc=256, ds_enc_hid=(64,), ds_agg_hid=(64,), dropout=0.15, d_attn=64):
        encoder = MnistEncoder(ds_enc_hid, d_enc, dropout)
        aggregator = agg.MultiHeadAttentionAggregator(1, d_enc, ds_agg_hid, d_attn, MNIST_N_CLASSES, dropout)
        super().__init__(device, MNIST_N_CLASSES, MNIST_N_EXPECTED_DIMS, encoder, aggregator)
...
class MnistGNN(models.ClusterGNN):
