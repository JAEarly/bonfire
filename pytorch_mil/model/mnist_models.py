...
def get_model_clz_from_name(model_name):
    if model_name == 'InstanceSpaceNN':
        return MnistInstanceSpaceNN
    if model_name == 'EmbeddingSpaceNN':
        return MnistEmbeddingSpaceNN
    if model_name == 'AttentionNN':
        return MnistAttentionNN
    if model_name == 'GNN':
        return MnistGNN
    raise ValueError("No MNIST model class found for model name {:s}".format(model_name))
